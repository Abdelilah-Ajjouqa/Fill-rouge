...
export const MembersPage = () => {
        ...
        try {
            if (modalMode === 'create') {
                ...
                if (createMember.fulfilled.match(result)) {
                    ...
                    setModalError(typeof result.payload === 'string' ? result.payload : 'Failed to create member.');
                }
            } else if (selectedMember) {
                const result = await dispatch(updateMember({ id: selectedMember._id, data: payload }));
                if (updateMember.fulfilled.match(result)) {
                    toast.success('Member updated successfully.');
                    closeModal();
                } else {
                    setModalError(typeof result.payload === 'string' ? result.payload : 'Failed to update member.');
                }
            }
        } finally {
    ...
    const handleDelete = async (member: Member) => {
        const confirmed = window.confirm(`Remove ${member.firstName} ${member.lastName}?`);
        if (!confirmed) {
            return;
        }

        const result = await dispatch(deleteMember(member._id));
        if (deleteMember.fulfilled.match(result)) {
            toast.success('Member deleted successfully.');
        } else {
            toast.error(typeof result.payload === 'string' ? result.payload : 'Failed to delete member.');
        }
    };
        ...
        return (
            <div className="p-8">
                <div className="bg-slate-900 border border-white/10 p-6">
                    ...
                    <p className="text-white/40 text-sm mt-2">This view is available for gym admins only.</p>
                </div>
            </div>
        );
    }
        ...
        <div className="p-8 space-y-8 animate-fade-in">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div>
                    ...
                    <p className="text-white/40 text-sm mt-1">Manage memberships for your gym.</p>
                </div>
                <button
            ...
            <div className="bg-slate-900 border border-white/10 p-6">
                ...
                {isLoading ? (
                    ...
                    <div className="p-6 border border-dashed border-white/10 text-white/40 text-sm">
                        No members found yet. Add your first member to get started.
                    </div>
                ) : (
