...
export const SuperAdminSettingsPage = () => {
    ...
    const accChange = (f: keyof AccountFormState) => (e: ChangeEvent<HTMLInputElement>) => setAccountForm((p) => ({ ...p, [f]: e.target.value }));
    ...
    const handleAccountSubmit = async (e: FormEvent) => {
        ...
        } finally { setIsAccountSaving(false); }
    };

    if (!isSuperAdmin) return <div className="p-8"><div className="bg-slate-900 border border-white/10 p-6"><h2 className="text-lg font-bold">Settings</h2><p className="text-white/40 text-sm mt-2">Super Admins only.</p></div></div>;

    return (
        <div className="p-8 space-y-8 animate-fade-in">
            <div><h2 className="text-2xl font-bold tracking-tight">Settings</h2><p className="text-white/40 text-sm mt-1">Manage your Super Admin profile.</p></div>
                ...
                <div className="bg-slate-900 border border-white/10 p-6">
                    ...
                    <form onSubmit={handleAccountSubmit} className="space-y-5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <InputField label="First Name" value={accountForm.firstName} onChange={accChange('firstName')} required />
                            <InputField label="Last Name" value={accountForm.lastName} onChange={accChange('lastName')} required />
                        </div>
                        <InputField label="Email" type="email" value={accountForm.email} onChange={accChange('email')} required />
                        <InputField label="New Password (optional)" type="password" value={accountForm.password} onChange={accChange('password')} placeholder="Leave blank to keep current" />
