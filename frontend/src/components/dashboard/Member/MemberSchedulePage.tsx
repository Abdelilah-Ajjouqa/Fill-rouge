...
export const MemberSchedulePage = () => {
  ...
  const sessions = useMemo(
    () => buildMemberScheduleEntries(activeSubscriptions),
    [activeSubscriptions],
  );
  ...
  const nextSessionSubtitle = nextSession
  ...
  return (
    <div className="p-8 space-y-8 animate-fade-in">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="text-white/40 text-sm mt-1">
            Keep track of your upcoming sessions and weekly timetable.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <StatCard
          title="Next Session"
          value={isLoading ? '...' : nextSessionValue}
          icon={<Clock className="h-5 w-5" />}
          subtitle={isLoading ? 'Fetching sessions' : nextSessionSubtitle}
          delay={300}
        />
      </div>
      ...
      <div className="bg-slate-900 border border-white/10 p-6">
        ...
        {isLoading ? (
          ...
          <div className="p-6 border border-dashed border-white/10 text-white/40 text-sm">
            No sessions scheduled yet. Check back after enrolling in activities.
          </div>
        ) : (
